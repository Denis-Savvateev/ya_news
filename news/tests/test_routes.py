...
class TestRoutes(TestCase):
    """Тестирует пути приложения news."""
...
# Вместо этого блока используем подход, основанный на субтестах. См ниже.
    # def test_home_page(self):
    #     """Проверь доступность главной страницы проекта."""
    ...
    #     """Проверь доступность записи новости по id."""
    #     url = reverse('news:detail', args=(self.news.id,))
    #     # Можно передать id(pk) и следующим образом, явно указав kwarg:
    ...
    def test_pages_availability(self):
        ...
        urls = (
            ...
            # Страницы лоина, логаута и регистрации также
            # проверяем в этом тесте.
            ('users:login', None),
            ('users:logout', None),
            ('users:signup', None),
        )
    ...
    def test_availability_for_comment_edit_and_delete(self):
        ...
        users_statuses = (
            (self.author, HTTPStatus.OK),  # автор комментария
                                           # должен получить ответ OK,
            (self.reader, HTTPStatus.NOT_FOUND),  # читатель должен
                                                  # получить ответ NOT_FOUND.
        )
    ...
    def test_redirect_for_anonymous_client(self):
        ...
        for name in ('news:edit', 'news:delete'):
            with self.subTest(name=name):
                ...
                # в котором передаётся адрес страницы, с которой
